...
<form action="{{route($route.'.index')}}" id="filter_form">
    <div class="row text-center" style="margin-bottom: 15px;">
        ...
        || isset($_GET['date_type'])|| isset($_GET['operation_type'])|| isset($_GET['customer_type']) ?'' :'hidden'}}>



<div class="col-md-3">
    <div class="input-group input-daterange">
        <label for="brands">{{__('Date Type')}}</label>
        <select name="date_type" class="form-control select2" id="date_type" style="width: 50%">
            <option value="">{{__('Select')}}</option>
            <option <?= isselected('sample_date',$_GET['date_type']??'') ?> value="sample_date">{{__('Sample Date')}}</option>
            <option <?= isselected('fromlab_date',$_GET['date_type']??'') ?> value="fromlab_date">{{__('To C.S. Date')}}</option>
        </select>
    </div>
</div>
        ...
        <div class="col-md-3" >
            ...
            <select name="operation_type" class="form-control select2" id="operation_type" style="width: 50%">
                <option value="">{{__('Select')}}</option>
                <optgroup label={{__('Colors')}}>
                    @foreach($colorsCategory as $singleColor)
                  
                    <option value="{{ $singleColor->CategoryCol_name }}" {{ (collect(old('operation_type'))->contains($singleColor->CategoryCol_name)) ? 'selected':'' }}>
                        {{ $singleColor->CategoryCol_code." - ".$singleColor->CategoryCol_name}}</option>
                    ...
                    <optgroup label={{__('Fashion')}}>
                        @foreach($FashionCategory as $singleFashion)
                        <option value="{{ $singleFashion->fascategory_name }}" {{ (collect(old('operation_type'))->contains($singleFashion->fascategory_name)) ? 'selected':'' }}>
                            {{ $singleFashion->fascategory_code." - ".$singleFashion->fascategory_name}}</option>
