...
<form action="{{route($route.'.index')}}" id="filter_form">
    <div class="row text-center" style="margin-bottom: 15px;">
        ...
        || isset($_GET['date_type'])|| isset($_GET['customer_type']) ?'' :'hidden'}}>
...
<div class="col-md-3">
    <div class="input-group input-daterange">
        ...
        <select name="date_type" class="form-control select2" id="date_type" style="width: 50%">
            ...
            <option <?= isselected('ReceiptDate',$_GET['date_type']??'') ?> value="ReceiptDate">{{__('Receipt Date')}}</option>
            <option <?= isselected('lab_receiptdate',$_GET['date_type']??'') ?> value="lab_receiptdate">{{__('Lab Receipt Date')}}</option>
            <option <?= isselected('fromlab_date',$_GET['date_type']??'') ?> value="fromlab_date">{{__('From Lab Date')}}</option>
            <option <?= isselected('DeliveryDate',$_GET['date_type']??'') ?> value="DeliveryDate">{{__('Delivery Date')}}</option>
        </select>
    </div>
</div>
