    ...
    <div class="row">
        <div class="col-12">
            <div class="header-index">
                <h3 class="fas fa-users">  {{__('view Deleted Customers')}}</h3>
             
            </div>
            <div class="col-12 mt-5 mb-3" id="searchbox" >
             @include('core::search',['route'=>'Customers.trashed'])
             <span class="note">{{__('accepts arabic and english letters and numbers')}}</span>
            </div>
          
            <div class="table-responsive mt-4 data">
                 
                @include('customers::Customers/deleted_Customers_table',['customers'=>$customers]) 
            </div>   
            {{-- ---------------------------------------------------------------------------------- --}} 
...
<script type="text/javascript">
    ...
    function restoreCustomer(id) {
        ...
        var url = '{{route('Customers.restore','id')}}'
        url = url.replace('id', id)
        swal.fire({
            title: "{{__('Are you sure To Restore This Customer?')}}",
            icon: "warning",
