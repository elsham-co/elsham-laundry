    ...
    {{-- <div class="row"> --}}
        <div class="col-12">
            <div class="header-index">
                <h3 class="fa fa-vest-patches">  {{__('Trashed Fashions Category')}}</h3>
             
            </div>
            <div class="col-12 mt-5 mb-3" id="searchbox" >
             @include('core::search',['route'=>'fascategory.trashed'])
             <span class="note">{{__('accepts arabic and english letters and numbers')}}</span>
            </div>
          
            <div class="table-responsive mt-4 data">
                 
                @include('productioncomponents::Fashions/deleted_FashionCategory_table',['Allfascategory'=>$Allfascategory]) 
            </div>   
            {{-- ---------------------------------------------------------------------------------- --}} 
...
<script type="text/javascript">
    ...
    function restoreFasCategory(id) {

        var url = '{{route('fascategory.restore','id')}}'
        url = url.replace('id', id)
        swal.fire({
            title: "{{__('Are you sure To Restore This Fashions Category?')}}",
            icon: "warning",
...
$("#custom-cm__item1").click(function() {
    window.location.href = "{{route('fascategory.index')}}";
});
