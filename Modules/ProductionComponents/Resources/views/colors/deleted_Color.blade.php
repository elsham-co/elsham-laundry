    ...
    {{-- <div class="row"> --}}
        <div class="col-12">
            <div class="header-index">
                <h3 class="fa fa-flask">  {{__('view Deleted Colors')}}</h3>
             
            </div>
            <div class="col-12 mt-5 mb-3" id="searchbox" >
             @include('core::search',['route'=>'colors.trashed'])
             <span class="note">{{__('accepts arabic and english letters and numbers')}}</span>
            </div>
          
            <div class="table-responsive mt-4 data">
                 
                @include('productioncomponents::colors/deleted_Color_table',['Colors'=>$Colors]) 
            </div>   
            {{-- ---------------------------------------------------------------------------------- --}} 
...
<script type="text/javascript">
    ...
    function restoreColor(id) {
        ...
        var url = '{{route('colors.restore','id')}}'
        url = url.replace('id', id)
        swal.fire({
            title: "{{__('Are you sure To Restore This Color?')}}",
            icon: "warning",
