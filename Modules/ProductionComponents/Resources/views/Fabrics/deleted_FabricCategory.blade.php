...
@endpush
@section('title')
    {{__('view Deleted Fabrics Category')}}
@endsection
@section('content')
    ...
    {{-- <div class="row"> --}}
        <div class="col-12">
            <div class="header-index">
                <h3 class="fas fa-signature">  {{__('view Deleted Fabrics Category')}}</h3>
             
            </div>
            <div class="col-12 mt-5 mb-3" id="searchbox" >
             @include('core::search',['route'=>'Category.trashed'])
             <span class="note">{{__('accepts arabic and english letters and numbers')}}</span>
            </div>
          
            <div class="table-responsive mt-4 data">
                 
                @include('productioncomponents::Fabrics/deleted_FabricCategory_table',['Allfabcategory'=>$Allfabcategory]) 
            </div>   
            {{-- ---------------------------------------------------------------------------------- --}} 
...
<script type="text/javascript">
    ...
    function restoreFabCategory(id) {

        var url = '{{route('Category.restore','id')}}'
        url = url.replace('id', id)
        swal.fire({
            title: "{{__('Are you sure To Restore This Fabric Category?')}}",
            icon: "warning",
