...
class ThreadsExportTrashed implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    protected $user;
...
UserRepositoryEloquent $user)
    ...
    {
        return [[__('Deleted Threads Report')],[__('Thread_ID'),__('Thread_Name'),__('Deleted_by')]];
    }

    public function collection()
    {
        $threadsExpert = $this->thread_info->onlyTrashed()->orderBy('id','desc')->get();
        ...
        foreach($threadsExpert as $threadExpert)
        {
            ...
            $data['deleted_by'] = $this->user->where('id',$threadExpert->deleted_by)->first()?$this->user->where('id',$threadExpert->deleted_by)->first()->username:'';
