...
class Storepro_follow_upService
{
    ...
    public function get_Movement($data = null)
    {
        ...
        ->where('stores.stage1', '<>', 'تسليم للعميل')
        ->select('stores.*')
        ->distinct();
        ...
        if(isset($data['user_list'])){
            $follow_up = $follow_up->where('stores.stage1',$data['user_list']);
        }
        // if(isset($data['user_list'])){
    ...
    public function delivery_store($data = null)
    {
    ...
    public function quality_and_deliveries($data = null)
    {
