...
<form action="{{ route('movements.xlsx')}}" id="filter_form">
    ...
    <div class="row" id="filter_components" {{isset($_GET['date_from']) || isset($_GET['date_to']) || isset($_GET['user_list']) || isset($_GET['fabric']) 
    || isset($_GET['colors'])|| isset($_GET['customer_type']) ?'' :'hidden'}}>
...
@endcan
                    ...
                    {{-- <button id="export_btn" type="submit" class="btn btn-success mt-2"> {{__('Export')}}</button> --}}
                    <button type="submit" class="btn btn-success mt-2"> {{__('Export')}}</button>
