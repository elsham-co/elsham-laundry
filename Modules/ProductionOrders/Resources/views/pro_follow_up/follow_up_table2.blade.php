      ...
      @foreach($Movements['stores'] as $key=> $Singale_movement)
          ...
          <tr style="text-align:center">
             ...
             <td>{{ $Singale_movement->Color_info }}</td>
              ...
              <td>@if( $Singale_movement->store1==1 )   {{ $Singale_movement->stage1 }} {{ $Singale_movement->location }} @else <p style="background: rgb(25, 167, 255);text-align: center">--</p> @endif</td>
        ...
        @canany(['create_transaction','view_active_order']) 
              <td>
                ...
                @can('edit_active_order')
                  <a href="{{route('activeorder.edit',$Singale_movement->production_order)}}"  class="btn btn-primary btn-sm" title={{__('transformation')}}>
