...
// return $request->all();
    ...
    public function index_delivery_store(Request $request,Storepro_follow_upService $service)
    {
        $delivery_store = $service->delivery_store($request->all());
        $CustomerName  = $service->getCustomerName();
        $FabricName  = $service->getFabricName();
        $ColorName  = $service->getColorsCategoryName();
        $UserName = $service->getUserName();
        ...
        if($request->ajax()){
            return view('productionorders::delivery_store/delivery_store_table')->with('delivery_store',$delivery_store);
        }
        return view('productionorders::delivery_store.index_delivery_store')->with('delivery_store',$delivery_store)->with('UserName',$UserName)
        ->with('CustomerName',$CustomerName)->with('FabricName',$FabricName)->with('ColorName',$ColorName);
    }

    public function index_quality_and_deliveries(Request $request,Storepro_follow_upService $service)
    {
        $quality_deliveries = $service->quality_and_deliveries($request->all());
        $CustomerName  = $service->getCustomerName();
        $FabricName  = $service->getFabricName();
        $ColorName  = $service->getColorsCategoryName();
        $UserName = $service->getUserName();
        ...
        if($request->ajax()){
            return view('productionorders::delivery_store/quality_and_deliveries_table')->with('quality_deliveries',$quality_deliveries);
        }
        return view('productionorders::delivery_store.index_quality_and_deliveries')->with('quality_deliveries',$quality_deliveries)->with('UserName',$UserName)
        ->with('CustomerName',$CustomerName)->with('FabricName',$FabricName)->with('ColorName',$ColorName);
    }
...
// return $request->all();
    ...
    public function edit_location($activlocation,Edit_follow_upServices $service,Storepro_follow_upService $service1)
    {
        $CustomerName1  = $service1->getCustomerName();
        $FabricName  = $service1->getFabricName();
        $ColorsName  = $service1->getColorName();
        $allorderdata = $service->editFollowp($activlocation);
        return view('productionorders::pro_follow_up.edit_location')->with('allorderdata',$allorderdata)
        ->with('CustomerName1',$CustomerName1)->with('FabricName',$FabricName)->with('ColorsName',$ColorsName);
    }



    public function update_location(Request $request,Updatepro_follow_upService $service,$activlocation)
    // public function update_activeorder(Request $request)
    {
        $service->update_activelocation($activlocation,$request->all());
        session()->put('success',__('Samples Order updated Successfully'));
    ...
    public function show($production_order)
    {
        ...
        return view('productionorders::Transaction_model.index_transaction_show')->with('transaction',$transaction);
    }
