...
class libraTransactionExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents
{
    protected $user;
...
FabricInfoRepositoryEloquent $Fabric_info)
    ...
    {
        return [[__('Follow-up Movement')],[__('Production Order'),__('Number Voucher'),__('Customer Name'),__('Colors Name'),
        __('Fabrics Name'),__('Fabrics Pieces'),__('Fashion Count'),__('Hall Name'),__('Stage Type'),__('Created_at')]];
    ...
    public function collection()
    {
        ...
        foreach($movementsExpert as $moveExpert)
        {
            ...
            $data['Hall Name'] = $this->user->where('id',$moveExpert->store1)->first()?$this->user->where('id',$moveExpert->store1)->first()->username:'';
            $data['Stage Type'] = $moveExpert->stage1;
