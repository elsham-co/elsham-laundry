...
Route::group(['middleware' => ['setLocale','auth']], function () {
    ...
    // Route::get('/colors/print', 'ColorStagesController@printColors')->name('colors.print');
        ...
        Route::get('/follow_up/create', 'StoreController@create')->name('pro_follow_up.create')
        ->middleware('permission:active_order');
        Route::post('/follow_up/store', 'StoreController@store')->name('pro_follow_up.store')
        ->middleware('permission:active_order');
        Route::get('/follow_up/edit/{color}', 'StoreController@edit')->name('pro_follow_up.edit')
        ->middleware('permission:create_transaction');
        Route::put('/follow_up/update/{color}', 'StoreController@update')->name('pro_follow_up.update')
        ->middleware('permission:create_transaction');
        ...
        Route::get('/delivery/delivery_store', 'StoreController@index_delivery_store')->name('delivery.index');
        Route::get('/delivery/quality_and_deliveries', 'StoreController@index_quality_and_deliveries')->name('quality_and_deliveries.index');

        Route::get('/transaction/show/{production_order}','StoreController@show')->name('transaction.show');

        Route::get('/follow_up/edit_location/{activeorder}', 'StoreController@edit_location')->name('activlocation.edit');
        Route::put('/follow_up/update_location/{activeorder}', 'StoreController@update_location')->name('activlocation.update');
